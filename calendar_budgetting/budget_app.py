...
class BudgetApp:
    ...
    def load_ui(self):
        ...
        tk.Label(self.root, text=f"Net Total: ${net_total:.2f}", font=("Arial", 14, "bold"), fg=color)\
            .grid(row=0, column=0, columnspan=7, pady=(5, 10))

        # Month navigation
        tk.Button(self.root, text="<", command=self.prev_month).grid(row=1, column=0, padx=10)
        tk.Label(self.root, text=f"{calendar.month_name[self.current_month]} {self.current_year}", 
                 font=("Arial", 16, "bold")).grid(row=1, column=1, columnspan=5)
        tk.Button(self.root, text=">", command=self.next_month).grid(row=1, column=6, padx=10)

        # Day labels
        days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
        for idx, day in enumerate(days):
            color = "#007BFF" if day in ["Sat", "Sun"] else "black"
            tk.Label(self.root, text=day, font=("Arial", 10, "bold"), fg=color).grid(row=2, column=idx, pady=2)
        ...
        for day in month_days:
            if day == 0:
                tk.Label(self.root, text="").grid(row=row, column=col)
            else:
                date_str = f"{self.current_year}-{self.current_month:02d}-{day:02d}"
                day_date = datetime.strptime(date_str, "%Y-%m-%d").date()
                bg = "#FFDDDD" if day_date == today else "#F0F0F0"
                ...
                for entry in self.get_entries(date_str):
                    tk.Button(frame, text=f"{entry[2]}: ${entry[3]}", font=("Arial", 8),
                              bg="#C3F3C3", command=lambda eid=entry[0]: self.edit_entry(eid))\
